...
class WorkerState:
    def __init__(self, idx: int, is_mapper: bool, addr: tuple[str, int]):
        ...
        self.addr: Final[tuple[str, int]] = (
            addr  # for udp connection ("localhost", port)
        )

        self.last_hb_recvd: int = 0  # when did we receive last heartbeat (in seconds)
        self.last_cp_id: int = (
            0  # (0 initially) the id of last checkpoint a worker made
        )
        self.recovery_id: int = 0
        self.is_done: bool = (
            False  # only for mappers (have mappers finished reading their stream)
        )
        self.last_checkpoint_done: bool = (
            False  # has a worker finished done its last checkpoint
        )
...
class CoordinatorState:
    ...
    def last_completed_checkpoint_id(self):
        ...
        if (
            checkpoint_id == 0
        ):  # this means there exist some worker has not checkpointed yet
            checkpoint_id = (
                -1
            )  # for this, we use -1 as id, which means recover from beginning
    ...
    def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
        ...
        for ws in state.workers.values():
            if ws.last_cp_id < self.checkpoint_id:
                ...
                break
        if all_acked:
            state.next_cp_id = self.checkpoint_id + 1
            return PHASE.CP
        return None
...
class RecoveryAckRecvMsg(RcvMsg):
    ...
    def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
        ...
        worker.recovery_id = self.recovery_id
        ...
        if state.phase == PHASE.RECOVER_REDUCER:
            ...
            all_recovered = True
            for ws in state.workers.values():
                if not ws.is_mapper and ws.recovery_id != state.next_recovery_id:
                    all_recovered = False
                    break
            if all_recovered:
...
class DoneRecvMsg(RcvMsg):
    def update(self, state: CoordinatorState, source: str) -> Optional[PHASE]:
        ...
        assert (
            state.workers[source].is_mapper == True
        ), "Only mappers should send DONE message"
...
class RecvThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            logging.debug(
                f"Received message of type '{message.msg_type.name}' from '{message.source}'"
            )
            msg = msg_factory(message)
            new_phase = msg.update(
                self.state, message.source
            )  # it will return new phase, in case there is some phase change
            if (
                new_phase is not None
            ):  # adding new phase into queue, so that send thread can read and take appropriate actions
...
class SendThread(threading.Thread):
  ...
  """
    ...
    def cp_phase(self):
        ...
        logging.info(f"{self.id} sending checkpoint markers to all the mappers")
        for _, ws in self.state.workers.items():
            if ws.is_mapper:
                CPMsg(self.state.next_cp_id, self.state.next_recovery_id).send(
                    self.state.sock, ws.addr
                )

    def recover_phase(self, is_mapper: bool) -> None:
        if is_mapper:
            assert self.state.phase == PHASE.RECOVER_MAPPER
            targets = self.state.workers.values()
            targets = [ws for ws in targets if ws.is_mapper]
        else:
            assert self.state.phase == PHASE.RECOVER_REDUCER
            targets = self.state.workers.values()
            targets = [ws for ws in targets if not ws.is_mapper]

        checkpoint_id = self.state.last_completed_checkpoint_id()
        recovery_id = self.state.next_recovery_id

        logging.info(f"{self.id} starting recovey")
        for target in targets:
            RecoveryMsg(checkpoint_id, recovery_id).send(self.state.sock, target.addr)

    def last_cp_phase(self):
        assert self.state.phase == PHASE.LAST_CP

        logging.info(f"{self.id} sending last checkpoint markers to both mappers")
        for _, ws in self.state.workers.items():
            if ws.is_mapper:
