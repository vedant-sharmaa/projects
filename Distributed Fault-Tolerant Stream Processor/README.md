# Distributed Fault-Tolerant Stream Processor
### Implementation of the Chandy-Lamport Algorithm for Consistent Global Checkpointing

This project is a high-performance distributed word-counting system modeled after **Apache Flink**. It implements asynchronous, consistent global snapshots using the **Chandy-Lamport algorithm** to ensure exact-state recovery in the event of worker failures.

---

## High Level System Overview.

The system performs word counting while being resilient to failures (except for the coordinator, which is assumed to be stable). Resiliency is achieved by using checkpoints where each worker creates a snapshot of its state. To ensure these snapshots are globally consistent, the system implements **Flink-like checkpointing**.
...
Here, we fix the number of mappers and reducers to 2 with 1
coordinator.  So the system looks like the following.

![HighLevelDesign](docs/system.png)
...
3. We assume that file system is always available and reliable. Checkpoints
